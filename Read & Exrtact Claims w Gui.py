...
# Prepare form functions
def first_button():
    working_directory = e1.get()
    PDF_of_Claims = e2.get()
    destination_directory = e3.get()
    ...
    with(Image(filename=PDF_of_Claims + ".pdf", resolution=200)) as source:
        images = source.sequence
        pages = len(images)
        for i in range(pages):
            Image(images[i]).save(filename='Cropped Images/' + PDF_of_Claims + str(i) + '.tiff')

    for page in range(0, pages):
        # Select page number to parse text from
        pagenumber = str(page)

        filetoworkon = 'Cropped Images/' + PDF_of_Claims + pagenumber + '.tiff'
        ...
        NumericFieldList = ("Patient Control Number", "Medical Record", "Type of Bill", "Fed Tax Number", "Statement From",
                            ...
                            "Pages Total Number", "Creation Date", "Total Value", "Total Value Decimals",
                            "Document Control No")
        ...
        extractionframe.loc[1, 'document'] = PDF_of_Claims
        ...
        print("Workbook {}, page {} successfully extracted".format(PDF_of_Claims, pagenumber))
...
Label(master, text="Enter Working Directory", font="Helvetica 8 bold italic").grid(row=1, column=1, padx=5, pady=2)
Label(master, text="Enter PDF filename", font="Helvetica 8 bold italic").grid(row=2, column=1, padx=5, pady=2)
Label(master, text="Enter Destination Directory", font="Helvetica 8 bold italic").grid(row=3, column=1, padx=5, pady=2)
...
Button(master, text='Submit Entries & Parse Pages', command=first_button).grid(row=4, column=1, columnspan=2, sticky=N, padx=5, pady=(5, 2))
